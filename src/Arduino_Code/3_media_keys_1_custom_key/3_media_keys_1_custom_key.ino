/*
  ...
  Dependencies:
  - HID-Project by NicoHood Version 2.8.4 or above
  ...
  The Reference links below is where the info to getting the project to work comes from
   @see https://uboopenfactory.univ-brest.fr/Les-Labs/MusicLab/Projets/Arduino-Media-Keys
  - most code comes from here.
  @see https://github.com/NicoHood/HID/wiki
  ...
  @see https://arduino.stackexchange.com/questions/8934/send-keyboard-media-keys-with-keyboard-library
  @see http://www.freebsddiary.org/APC/usb_hid_usages.php
  - list of hid codes for further use or differt use case
  @see https://www.instructables.com/id/USB-Volume-Control-and-Caps-Lock-LED-Simple-Cheap-/
  @see https://docs.arduino.cc/hacking/software/LibraryTutorial
...
/// @todo maybe move beacause of loading order of data and info from config
CRGB leds[NUM_LEDS];
...
void turnOnRGBByState(int cRed, int cGreen, int cBlue, bool state, int ledIndex) {
  if (ledIndex < 0 || ledIndex >= NUM_LEDS) {
    isDebugTruePrintToSerial("ledIndex out of range");
    return;  // Handle out-of-range index
  }

  switch (state) {
    case 0:  // Turn on LEDs with the specified color
      FastLED.clear();
...
/// Handles the button press events from Button::CheckButtonEvent() where:
/// Click = 1, Double-Click = 2, Hold = 3, Long Hold = 4
/// takes two KeyboardKeycode as argument from HID-Project.h for single press and double press for the button that is pressed
void pressEventHandler(int event, KeyboardKeycode key, KeyboardKeycode key2) {
  switch (event) {
    case 1:
      clickEvent();
...
/// Double press handler, called from pressEventHandler() can be used for other things, takes a KeyboardKeycode as argument from HID-Project.h
void doublePress(KeyboardKeycode key) {
  isDebugTruePrintToSerial("double press");
  Keyboard.write(key);  // Action to be performed on double press
...
void setup() {
  ...
  /// 1.5 second delay for recovery and HID connection is sometime unstable at start, this limits it to less. But may not be needed.
  delay(1500);
...
void loop() {
  ...
  pressEventHandler(b, buttonPin4Action1, buttonPin4Action2);  // this is only for button 4
}
